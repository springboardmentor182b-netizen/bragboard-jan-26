...
const CreateShoutOut = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            // Hardcoded sender_id=1 and recipient_id=2 for demo purposes
            // In a real app, this would come from auth context and user selection
            await axios.post('http://localhost:8000/shoutouts/?sender_id=1', {
                content: message,
                recipient_id: 2, // Hardcoded recipient
                tags: []
            });
            setMessage('');
            setRecipient('');
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <div className="relative">
                            <input
                                type="text"
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-brand-orange focus:ring-2 focus:ring-brand-orange/20 outline-none transition-all pl-10"
                            ...
                            <button
                                type="button"
                                className="absolute right-2 top-2 bg-brand-orange text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-orange-600 transition-colors"
                            >
                                Add
                            </button>
                        </div>
