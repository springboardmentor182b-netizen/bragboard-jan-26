...
from src.auth.utils import verify_password, create_access_token, get_current_user
...
def forgot_password(request: schemas.ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    # Return questions for this user
    questions = [{"id": q.id, "question": q.question} for q in db_user.security_questions]
...
def reset_password(request: schemas.ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    # Double check security answer for safety in one call (stateless-ish)
    verified = False
